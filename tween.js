...
function Tween(_args) {
  ...
  if (typeof this.duration != "number")
    return alert("Tween: Duration is not a number");
  if (typeof this.properties != "object")
    return alert("Tween: Properties is not an object");

  // setup properties
  for (var _key in this.properties) {
    var _prop = this.properties[_key];
    if (typeof _prop == "number")
      _prop = this.properties[_key] = { end: _prop };
    if (typeof _prop != "object")
      return alert("Tween: Property " + _key + " is not the correct format");
    ...
    if (
      ...
      _prop.start.toString().match(/^\d+\.\d+$/)
    ) {
      _prop.start = parseFloat(_prop.start);
    } else if (
      typeof _prop.start != "number" &&
      _prop.start.toString().match(/^\d+$/)
    ) {
      _prop.start = parseInt(_prop.start, 10);
    }

    if (
      typeof _prop.end != "number" &&
      _prop.end.toString().match(/^\d+\.\d+$/)
    ) {
      _prop.end = parseFloat(_prop.end);
    } else if (
      typeof _prop.end != "number" &&
      _prop.end.toString().match(/^\d+$/)
    ) {
      _prop.end = parseInt(_prop.end, 10);
    }

    if (typeof _prop.start != "number")
      return alert("Tween: Property " + _key + ": start is not a number");
    if (typeof _prop.end != "number")
      return alert("Tween: Property " + _key + ": end is not a number");
    if (_prop.filter && typeof _prop.filter != "function")
      return alert("Tween: Property " + _key + ": filter is not a function");
  }
}
...
Tween.prototype.logic = function (clock) {
  ...
  for (var _key in this.properties) {
    ...
    var _value =
      _prop.start +
      ease(this.amount, this.mode, this.algorithm) * (_prop.end - _prop.start);
